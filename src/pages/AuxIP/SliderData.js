        ...
        <Slider className="slider-area slider-style-4 slider-dot rn-slick-dot rn-slick-arrow" {...BannerActivation}>
            {setSliderData(props.content.Slider).map((data, index) => (
                <div key={index} className="single-slide">
                    ...
                    <div className={` ${(authenticiation()===true)? "mydivouter" : ""} mt-5`}>
                        {(authenticiation()===true)? <button type="button" className="mybuttonoverlap btn btn-primary" onClick={() => showPopUp(data.slider_slider_img[0].id)}><i className="fa fa-pencil" aria-hidden="true"></i></button> : ""}
